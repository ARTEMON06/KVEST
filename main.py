...
texts = {
    'start_1': """"Скоро мы доедем?" - спросила Кэтрин.
...
Кэтрин и Поли жаждут пойти купаться. Мотор барахлит, лишь бы моя Импала не подвела... """,
    'start_2': """Добравшись, я с Томми пошел расставлять палатку. Девушки побежали к озеру. Томми говорит, 
что маньяк, которого разыскивает его отдел расследований совершил уже 12 убийств за пару суток, кличка его - 
Бродяга. Закончив с палаткой, надо было сходить и набрать воды. Я вернулся с целой бутылкой. Из палатки торчали 
башмаки Томми, я подумал, что он уснул, но зайдя, остолбенел... Томми, а точнее его тело лежало по середине 
палатки, вокруг него лужа густой  крови. Его точно кто-то зарезал, но кто? Кто может так свирепо убить человека 
посреди дня еще и в лесу. Что делать? Как мне быть?""",
...
Мне стало безумно страшно. Я рассказал ей про Томми. Она заплакала и впала в панику...""",
    'step_2': """Кэтрин побежала заводить машину, я догнал ее и помог. Убедившись, что она в порядке, я, 
...
хотелось вернуть все назад и не поехать на этот пикник. Погрузившись в себя, я прошел пару десятков метров и 
понял, что заплутал.""",
...
некоторое время. Я боюсь оставлять ее одну. """,
    'finish_6': """Я все таки решился начать разговор первый. Я не знал, как себя вести и как не сделать Кэти еще 
хуже. Но нам нужно было обдумать, что же делать дальше. \"Кэти... Я понимаю, как тебе сейчас плохо, 
но нам необходимо обсудить, что делать дальше. Прости меня!\" Кэтрин лишь всхлипнула и моё сердце сжалось. Она 
повернула голову и дала знак, что я могу продолжать. \"Я думаю нам стоит сразу же по приезду в город сообщить в 
службу безопасности\". Кэт лишь кивнула. Мы спокойно продолжили ехать... """,
    'preface': """Эта игра создана для любителей маленьких игр с остреньким сюжетом. Если Вы хотите выйти из игры на 
любом её этапе напишите просто "выход". 
...
def pprint(text_to_print, speed=0.035):
...
def start():
    clear_term()
    # print(texts['start_1'])
    pprint(texts['start_1'])
    pprint(texts['start_2'])
    time.sleep(1)
